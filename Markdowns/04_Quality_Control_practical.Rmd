...
information about the distribution of reads within each gene in order to assess
RNA integrity. We will also gather some QC metrics from the Salmon outputs, such
as alignment rate and insert size.

## 2. Detailed metrics with Picard Tools
...
The first thing we will need to do is construct the refFlat file. We can do this
using the data contained in the bam file header. The file
`create_refflat_from_sam_header.py` in the **scripts** directory is a python
script  that will read a bam file and generate the required refFlat file from
the header. For future reference, you can download the script from our
[GitHub repository](https://github.com/crukci-bioinformatics/BamHeaderToRefflat).

In addition to the python script, you will also need to have
...
to identify the coding regions of the genome. The refFlat file is one format of
gene annotation, where each line contains the information for the locations of
each exon of a particular transcript on the genome. The Picard tool 
`CollectRnaSeqMetrics` normally uses this file to translate  the genomic 
alignments of reads into details of the genes to which the original mRNA
pertained. To analyse the alignments from Salmon, we still need to provide a
refFlat file, which can be generated from the header of the bam file.

The script `create_refflat_from_sam_header.py` will do this for us and requires
two parameters:
...
create_refflat_from_sam_header.py \
  -b salmon_qc_demo/SRR7657883/SRR7657883.salmon.bam \
...
long for the practical. You should find a file called `SRR7657883.salmon.bam`
under the directory `salmon_qc_demo/SRR7657883`, please use this file rather
...
java -jar picard/picard.jar CollectRnaSeqMetrics \
          INPUT=salmon_qc_demo/SRR7657883/SRR7657883.salmon.bam \
          OUTPUT=salmon_qc_demo/SRR7657883/SRR7657883.salmon.RNA_metrics.txt \
...
>
> If you wish to look at the metrics file, the easiest way is to open it in a
> spreadsheet. On the course machines we have LibreOffice Calc. You can find
> this in the launcher bar at the bottom of the desktop.
...
> In the `salmon` directory you should find Salmon outputs and RNAseq metrics 
> for all of the sampless.  
>
