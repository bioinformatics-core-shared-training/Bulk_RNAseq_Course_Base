...
>       * **File containing the #1 mates** - *SRR7657883.subset_2M.sra_1.fastq.gz*
>       * **File containing the #2 mates** - *SRR7657883.subset_2M.sra_2.fastq.gz*
...
By default the samtools output is in SAM format so that the results can be read
by a human, in this case the `-b` flag tells it to output in BAM format.
...
> 1. Transform your aligned SAM file in to a BAM file called
> `SRR7657883.salmon.bam`. Use the option `-@ 7` to use 7 cores, this vastly
