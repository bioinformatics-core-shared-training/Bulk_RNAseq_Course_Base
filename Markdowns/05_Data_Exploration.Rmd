...
DESeq2 provides two additional functions for transforming the
data:
...
We can examine how the mean-variance relationship has been affected by this transformation:
...
Above, we've applied the `rlog()` transformation to the data. 

1. Apply the VST transformation using the `varianceStabilizingTransformation()` function.
2. Examine the mean-variance relationship after the VST transformation. How does it compare to the `rlog()` transformation?

<details><summary>Answer</summary>

We can apply the VST transformation to our filtered counts matrix:
...
We can also explore the mean-variance relationship after this transformation:
...
The mean-variance relationship after the VST transformation achieves its desired outcome: the variance is approximately constant across the range of mean expression values. 

This is similar to the effect of the `rlog()` transformation, however there is no obvious shrinkage of the variance for low count genes. 
The shrinkage of variance for lowly expressed genes is a distinctive feature of the `rlog()` transformation, which attempts to remove potential noise observed for low-count genes.
</details>
:::
...
The plot we have generated shows us the first two principal components. This
shows us the relationship between the samples according to the two greatest
sources of variation. Sometimes, particularly with more complex experiments 
with more than two experimental factors, or where there might be confounding
factors, it is helpful to look at more principal components.

1. Redraw the plot, but this time plot the 2nd principal component on the 
x-axis and the 3rd prinicple component on the y axis. To find out how to do
the consult the help page with `?autoplot.prcomp`.
2. Examine the `summary(pcDat)` to find how much of the variance is explained by the principal components. Do you think 2 principal components are sufficient to capture the main sources of variation in the data? Why or why not?

<details><summary>Answer</summary>

Examining the help for the function we can see that we can specify which principal components to plot using the `x` and `y` arguments:
...
We can examine the variance explained by the principal components using the `summary` function:

```{r pca_summary}
summary(pcDat)
```

The first two principal components account for a total of 77% of the variance in the data, which is a very large proportion. 
Further principal components account for much smaller proportions of the variance, so it is likely that the first two principal components are sufficient to capture the main sources of variation in the data.

We could optionally visualise the variance explained by each principal component using a scree plot:

```{r screeplot}
plot(summary(pcDat)$importance[2,] * 100, 
     type = "b", 
